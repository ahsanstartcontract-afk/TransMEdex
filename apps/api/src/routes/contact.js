...
router.post('/', contactRateLimit, async (req, res) => {
	try {
		...
		const { firstName, lastName, email, phone, practiceName, numberOfPhysicians, message } = sanitized;
		const mailTo = process.env.MAIL_TO || process.env.SMTP_USER;
		const smtpUser = process.env.SMTP_USER;

		// ── Send notification to TransMedex team ──────────────
		await transporter.sendMail({
			from: `"TransMedex Website" <${smtpUser}>`,
			to: mailTo,
			replyTo: email,
			subject: `New Contact Form — ${firstName} ${lastName} (${practiceName})`,
		...
		await transporter.sendMail({
			from: `"TransMedex" <${smtpUser}>`,
			to: email,
			subject: 'Thank you for contacting TransMedex!',
			html: contactAutoReplyHtml({ firstName }),
		});
